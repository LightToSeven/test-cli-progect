    ...
    <div class="app">
        ...
        <div class="app__list">
            <div class="app__top--control">
                ...
                -->
                <input type="text" v-model='searchQ' class="app__top--search" placeholder="Поиск">
            </div>
            <div class="app__item--wrap">
                <!-- принимаем события удаления контакта и вызываем метод удаления контакта  -->
                <list-item v-for='contact in filteredContacts' :key='contact.id' :ref='"item-" + contact.id' @checkbox-text='checkboxTexts' @delete-contact='deleteContact' @edit-contact='editContact' :contact='contact'>
                </list-item>
            </div>
        </div>
        <!--
    ...
    export default {
        ...
        computed: {
            filteredContacts() {
                ...
                const filteredContacts = contactsList.filter(el => {
                    return el.text.toLowerCase().indexOf(searchQ) !== -1;
                })
        ...
        methods: {
            ...
            contactEdit(contact) {
                for(const [index, value] of this.contactsList.entries()) {
                    if (value.id === contact.id) {
                        this.contactsList[index].text = contact.text;
                    }
                }
            },
            checkboxTexts(contact) {
                console.log(contact);
                for(const [index, value] of this.contactsList.entries()) {
                    if (value.id === contact.id) {
                        if(contact.checkbox){
                            this.contactsList[index].checkbox = false;
                        }else{
                            this.contactsList[index].checkbox = true;
                        }
                    }
                }
            },
            deleteContact(contact) {
                ...
                const message = `Вы уверены что хотите удалить Текст : ${contact.text}`
                const options = {
                    okText: 'Удалить',
                    cancelText: 'Отменить',
                };
                ...
                this.$dialog.confirm(message, options).then(() => {
                    for (const [index, value] of this.contactsList.entries()) {
                        if (value.id == contact.id) {
                            ...
                            this.$notify({
                                group: 'foo',
                                title: `Контакт ${contact.text} успешно удален`
                            });
                            break;
                        }
                    }
            ...
            addContact(contact) {
                ...
                this.$notify({
                    group: 'foo',
                    title: `Контакт ${contact.text} успешно добавлен`
                });
            }
        },
        created() {
